...
1. **Configura las variables del servidor** en tu `.env` (o como variables de entorno en producción):
   ...
   - Si Auth0 solo tiene habilitados callbacks para dominios específicos (por ejemplo `localhost`), define `AUTH0_REDIRECT_URI` con la URL exacta autorizada. El cliente la reutilizará incluso si accedes a la aplicación desde otro dominio.
